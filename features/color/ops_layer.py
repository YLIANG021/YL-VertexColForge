...
def _rebuild_color_attribute(mesh, color_attr, target_name, target_type, target_domain):
    ...
    new_attr = mesh.color_attributes.new(name=target_name, type=target_type, domain=target_domain)
        ...
        with transactions.CleanupStack() as cleanup:
            ...
            if 0 <= idx < len(keys):
                ...
                if mesh.color_attributes:
                    new_keys = list(mesh.color_attributes.keys())
                    new_idx = min(idx, len(new_keys) - 1)
                    mesh.color_attributes.active_color_index = new_idx
                    mesh.color_attributes.render_color_index = new_idx
        ...
        try:
            with transactions.CleanupStack() as cleanup:
                ...
                if final_name in keys:
                    ...
                    mesh.color_attributes.render_color_index = active_index
        except ValueError as exc:
            self.report({"WARNING"}, str(exc))
            return {"CANCELLED"}

        display.refresh_after_color_write(context, mesh, final_name, obj=obj)
...
class MESH_OT_YLVCConvertDataType(bpy.types.Operator):
    ...
    def poll(cls, context):
        obj = context.active_object
        return active_mesh_has_color_attributes(context) and obj.mode == "OBJECT"
    ...
    def execute(self, context):
        ...
        display.refresh_after_color_write(context, mesh, new_attr.name, obj=obj)
